...
class YDarkSecurityMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, open_paths: Optional[List[str]] = None, simple_auth_paths: Optional[List[str]] = None):
        super().__init__(app)
        ...
        self.simple_auth_paths = simple_auth_paths or []
    
    async def dispatch(self, request: Request, call_next):
        # Skip security for open paths
        if request.url.path in self.open_paths:
            # For simple auth paths, still validate API key but skip HMAC
            if request.url.path in self.simple_auth_paths:
                return await self._handle_simple_auth(request, call_next)
    ...
    async def _handle_simple_auth(self, request: Request, call_next):
        """Handle simple authentication (only X-API-Key required)"""
        try:
            x_api_key = request.headers.get("X-API-Key")
            client_ip = request.client.host if request.client else "unknown"
            ...
            # Check basic rate limiting (optional for simple auth)
            if not await check_rate_limit(key_data, client_ip):
                return JSONResponse(
                    status_code=429,
                    content={
            ...
            # Update usage statistics for simple auth endpoints
            await update_usage_stats(key_data)
            ...
            response.headers["X-RateLimit-Remaining"] = str(max(0, RATE_LIMIT_RPS - key_data.daily_quota_used % RATE_LIMIT_RPS))
            response.headers["X-Daily-Quota-Remaining"] = str(max(0, DAILY_QUOTA - key_data.daily_quota_used))
            ...
            return JSONResponse(
                status_code=e.status_code,
                content={"error": e.detail, "code": "SIMPLE_AUTH_ERROR"}
            )
        except Exception as e:
            logger.error(f"Simple auth error: {e}")
            return JSONResponse(
                status_code=500,
                content={"error": "Internal auth error", "code": "INTERNAL_ERROR"}
            )
...
def secure_models_app(models_app: FastAPI, open_paths: Optional[List[str]] = None, simple_auth_paths: Optional[List[str]] = None) -> FastAPI:
    """Add security middleware to models app"""
    
    # Add security middleware
    models_app.add_middleware(YDarkSecurityMiddleware, open_paths=open_paths, simple_auth_paths=simple_auth_paths)
